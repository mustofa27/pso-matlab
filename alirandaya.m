...
function [test, cost] = alirandaya (busdata,linedata)
basemva = 100;  accuracy = 0.001; accel = 1.6; maxiter = 100;

% -------------------------------------------------------------------------
% MEMBENTUK MATRIKS YBUS
% -------------------------------------------------------------------------
...
cost(1)=(0.02*(Pgg(1)^2))+(2*Pgg(1))+(0);     
cost(2)=(0.0175*(Pgg(2)^2))+(1.75*Pgg(2))+(0);  
cost(3)=(0.0625*(Pgg(3)^2))+(1*Pgg(3))+(0);  
cost(4)=(0.00834*(Pgg(4)^2))+(3.25*Pgg(4))+(0);     
cost(5)=(0.025*(Pgg(5)^2))+(3*Pgg(5))+(0);  
cost(6)=(0.025*(Pgg(6)^2))+(3*Pgg(6))+(0);
totalcost = sum(cost);

% evaluasi fitness
test = 100 / (totalcost+real(SLT));
cost = totalcost;
save datatotalcost totalcost SLT;
